...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      // Fetch folders excluding deleted entries, and only owner_id IS NULL (general folders)
      final generalData = await Supabase.instance.client
          .from('folders')
          .select()
          .eq('family_id', familyId)
          .eq('is_deleted', false) // <-- exclude soft-deleted folders
      ...
      if (mounted) {
        // Normalize and dedupe folders by name (case-insensitive) to avoid duplicate sections
        final fetchedFolders = List<Map<String, dynamic>>.from(generalData);
        final unique = <String, Map<String, dynamic>>{};
        for (var f in fetchedFolders) {
          final rawName = (f['name'] ?? '').toString();
          final key = rawName.trim().toLowerCase();
          if (!unique.containsKey(key)) {
            f['name'] = _normalizeCategoryName(rawName);
            unique[key] = f;
          }
        }
        ...
        setState(() {
          ...
          _generalFolders = unique.values.toList();
  ...
  void _copyFamilyId() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text("Family ID Copied!"), backgroundColor: Colors.green));
  }

  void _showInviteDialog() {
    if (_currentFamilyId.isEmpty) return;
    final invite = 'famvault://join/$_currentFamilyId';

    // deterministic 6-digit code derived from familyId
    int hash =
        _currentFamilyId.codeUnits.fold(0, (h, c) => (h * 31 + c) & 0xFFFFFFFF);
    final code = (hash.abs() % 1000000).toString().padLeft(6, '0');
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
              onPressed: () {
                Share.share('Join my Famvault family: $invite');
                Navigator.pop(ctx);
              },
  ...
  Future<void> _handleScan() async {
    ...
    if (result == null || result.pdf == null)
      return; // User cancelled or no PDF returned
  ...
  Future<void> _showSaveDialog(File file) async {
    ...
    if (_generalFolders.isNotEmpty)
      selectedFolderId = _generalFolders.first['id'];
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  isExpanded: true,
                  items: _generalFolders
                      .map((f) => DropdownMenuItem(
                          value: f['id'] as String, child: Text(f['name'])))
                      .toList(),
                  onChanged: (val) =>
  ...
  Future<void> _saveScan(File file, String fileName, String folderId) async {
    ...
    try {
      ...
      final storagePath =
          '${user.id}/${DateTime.now().millisecondsSinceEpoch}_$cleanName.$fileExt';
  ...
  // Normalize category/folder names to avoid 'general' vs 'General' and similar duplicates
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () async {
              ...
              if (user != null) {
                ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text("Generating Folders...")));
                try {
                  final memberData = await Supabase.instance.client
                      ...
                      .single();
                  await FamilyService()
                      .createPersonalFolders(user.id, memberData['family_id']);
                  ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text("Done! Pull to refresh.")));
                  _fetchData();
                } catch (e) {
                  ScaffoldMessenger.of(context)
          ...
          IconButton(
              onPressed: () => Supabase.instance.client.auth.signOut().then(
                      ...
                      return Card(
                        ...
                        child: ListTile(
                          leading: CircleAvatar(
                            backgroundColor:
                                isMe ? Colors.blue[100] : Colors.orange[100],
                          ...
                          subtitle:
                              Text(member['role'].toString().toUpperCase()),
                          trailing:
  ...
  Widget _buildFolderCard(String title, String iconString, String folderId) {
    return InkWell(
      onTap: () => Navigator.push(
          ...
          MaterialPageRoute(
              builder: (_) =>
                  FolderScreen(folderId: folderId, folderName: title))),
      child: Container(
