...
import '../family/family_avatar_uploader.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  String? _familyAvatarUrl; // current family display picture (dp_url) 
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      // fetch family dp_url
      final familyRow = await Supabase.instance.client.from('families').select('dp_url').eq('id', familyId).maybeSingle();
      final dp = familyRow != null ? familyRow['dp_url'] as String? : null;
  ...
  void _showInviteDialog() {
    ...
    int hash = _currentFamilyId.codeUnits.fold(0, (h, c) => (h * 31 + c) & 0xFFFFFFFF);
        ...
        content: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                SelectableText('Code: $code', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            tooltip: 'Edit Family Photo',
            onPressed: () async {
              if (_currentFamilyId.isEmpty) {
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('No family found'), backgroundColor: Colors.orange));
                return;
              }
              await showDialog(context: context, builder: (ctx) => AlertDialog(
                title: const Text('Edit Family Photo'),
                content: FamilyAvatarUploader(familyId: _currentFamilyId, currentUrl: _familyAvatarUrl),
                actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Close'))],
              ));
              // Refresh data to fetch new dp_url
              await _fetchData();
            },
          ),
          IconButton(
