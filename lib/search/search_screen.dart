...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void _runSearch(String query) async {
    ...
    PostgrestFilterBuilder filterBuilder = Supabase.instance.client
        .from('documents')
        .select();
    ...
    PostgrestTransformBuilder transformBuilder = filterBuilder
        .order('created_at', ascending: !_isNewestFirst)
        .limit(50);
  ...
  Future<void> _openFile(Map<String, dynamic> file) async {
     try {
       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Opening..."), duration: Duration(milliseconds: 800)));
       ...
       final url = await Supabase.instance.client.storage.from('documents').createSignedUrl(path, 3600);
       
       if (context.mounted) {
         Navigator.push(context, MaterialPageRoute(builder: (_) => FileViewerScreen(
           fileUrl: url, 
           fileName: name, 
           fileType: ext
         )));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: Icon(_isNewestFirst ? Icons.arrow_downward : Icons.arrow_upward),
